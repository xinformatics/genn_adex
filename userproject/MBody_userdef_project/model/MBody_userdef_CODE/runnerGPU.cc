...
\brief File generated from GeNN for the model MBody_userdef containing the host side code for a GPU simulator version.
*/
//-------------------------------------------------------------------------
...
void pushKCStateToDevice()
 {
    ...
    CHECK_CUDA_ERRORS(cudaMemcpy(d_wKC, wKC, 1000 * sizeof(float), cudaMemcpyHostToDevice));
    }
...
void pushPNKCStateToDevice()
 {
    size_t size = CPNKC.connN;
    CHECK_CUDA_ERRORS(cudaMemcpy(d_gPNKC, gPNKC, size * sizeof(float), cudaMemcpyHostToDevice));
    CHECK_CUDA_ERRORS(cudaMemcpy(d_EEEEPNKC, EEEEPNKC, 1000 * sizeof(float), cudaMemcpyHostToDevice));
...
void pullPNSpikeTimesFromDevice()
 {
    ...
    CHECK_CUDA_ERRORS(cudaMemcpy(sTPN, d_sTPN, glbSpkCntPN[0] * sizeof(unsigned int), cudaMemcpyDeviceToHost));
    }
...
void pullKCStateFromDevice()
 {
    ...
    CHECK_CUDA_ERRORS(cudaMemcpy(wKC, d_wKC, 1000 * sizeof(float), cudaMemcpyDeviceToHost));
    }
...
void pullLHISpikeTimesFromDevice()
 {
    //Assumes that spike numbers are already copied back from the device
    CHECK_CUDA_ERRORS(cudaMemcpy(sTLHI, d_sTLHI, glbSpkCntLHI[0] * sizeof(unsigned int), cudaMemcpyDeviceToHost));
    }
...
void pullPNKCStateFromDevice()
 {
    ...
    CHECK_CUDA_ERRORS(cudaMemcpy(EEEEPNKC, d_EEEEPNKC, 1000 * sizeof(float), cudaMemcpyDeviceToHost));
...
//model.padSumSynapseTrgN[model.synapseGrpN - 1] is 3456
    dim3 sThreads(96, 1);
    dim3 sGrid(36, 1);
    ...
    calcNeurons <<< nGrid, nThreads >>> (ratesPN, offsetPN, t);
    cudaEventRecord(neuronStop);
    cudaEventSynchronize(neuronStop);
    float tmp;
    cudaEventElapsedTime(&tmp, synapseStart, synapseStop);
    synapse_tme+= tmp/1000.0;
    cudaEventElapsedTime(&tmp, learningStart, learningStop);
    learning_tme+= tmp/1000.0;
    cudaEventElapsedTime(&tmp, neuronStart, neuronStop);
    neuron_tme+= tmp/1000.0;
    iT++;
    t= iT*DT;
    }
