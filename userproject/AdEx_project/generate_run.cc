...
//--------------------------------------------------------------------------
/*! \file userproject/AdEx_project/generate_run.cc
...
#ifdef _WIN32
  if (dbgMode == 1) {
    cmd = "devenv /debugexe model\\AdEx_sim.exe " + toString(argv[3]) + " " + toString(which);
  }
  else {
    cmd = "model\\AdEx_sim.exe " + toString(argv[3]) + " " + toString(which);
  }
#else // UNIX
  if (dbgMode == 1) {
    cmd = "cuda-gdb -tui --args model/AdEx_sim " + toString(argv[3]) + " " + toString(which);
  }
  else {
    cmd = "model/AdEx_sim " + toString(argv[3]) + " " + toString(which);
  }
#endif
  retval=system(cmd.c_str());
  if (retval != 0){
